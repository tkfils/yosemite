...
$result = mysql_query("SELECT i.title, i.description, i.pubdate, i.id, i.author, i.url, i.added, i.selections, c.title, i.comments FROM item i, channels c WHERE i.cid=c.id and i.added like '".$date_select."' and DATE_SUB(CURDATE(), INTERVAL 5 DAY) <= i.pubdate and (c.id=294 or c.id=424 or c.id=425 or c.id=426 or c.id=427 or c.id=428 or c.id=1066 or c.id=1084 or c.id=1085 or c.id=1086 or c.id=1087 or c.id=1088 or c.id=1089 or c.id=1090 or ((c.parent=16 or c.parent=24 or c.id=294 or c.id=309 or c.id=386 or c.id=311 or c.id=312 or c.id=305 or c.id=323) and (i.description like '%King%s Fund%' or i.description like '%Health manager%' or i.description like '%Clinical commissioning%' or i.description like '%GP commissioning%' or i.description like '%NHS%' or i.description like '%ntegrated health%' or i.description like '%social care%' or i.description like '%ntegrated care%' or i.description like '%ospital%' or i.description like '%patient%' or i.description like '%Doctor%' or i.description like '%nurse%' or i.description like '%health%' or i.description like '%Health Foundation%' or i.description like '%Anita Charlesworth%' or i.description like '%Jennifer Dixon%' or i.description like '%Nuffield Trust%' or i.description like '%Nigel Edwards%' or i.description like '%John Appleby%' or i.description like '%Candace Imison%'))) order by i.title asc",$db);
...
$title=str_replace('Richard Murray','<span style=background-color:red>Richard Murray</span>',$title);
...
$description=str_replace('Richard Murray','<span style=background-color:red>Richard Murray</span>',$description);
